...
qplot(x = friend_count, data = subset(pf, !is.na(gender)), binwidth = 20) +
  scale_x_continuous(limits = c(0, 1000),
                     breaks = seq(0, 1000, 50)) +
  facet_wrap(~gender)
```
...
```{r Tenure}
qplot(x=pf$tenure, data=pf, binwidth=30,
...
qplot(x=pf$age, data=pf, binwidth=5, 
  ...
  xlab('Age of usrs using Facebook') + 
  ylab('Number of users in sample') 
```
...
Notes:
```{r}
qplot(data = pf, x = pf$friend_count)
```
```{r}
...
p1 = qplot(data = pf, x = log10(pf$friend_count+1))
p2 <- ggplot(aes(x = pf$friend_count), data = pf) + 
  geom_histogram() +
  scale_x_log10()
